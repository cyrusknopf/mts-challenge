...
export default function LeaderboardTable({ entries }: LeaderboardTableProps) {
  ...
  const formatDate = (dateString: string) => {
    ...
    return `${date.toLocaleDateString()} ${date.toLocaleTimeString()}.${date.getMilliseconds().toString().padStart(3, '0')}`;
  };
    ...
    <div className="overflow-x-auto w-full rounded-lg shadow-xl">
      <div className="bg-nova-gray4/70 backdrop-blur-sm border border-nova-gray5 rounded-lg">
        <table className="w-full text-white border-collapse">
          <thead>
            <tr className="border-b border-nova-gray5/80">
            ...
            {entries.length > 0 ? (
              entries.map((entry) => (
                ...
                >
                  ...
                  <td className="py-4 px-6 text-right font-medium">{entry.points}</td>
                  <td className={`py-4 px-6 text-right font-medium ${entry.profit >= 0 ? 'text-green-400' : 'text-red-400'}`}>
